...
function GlobalThemeTab({ data, updateSection }) {
  const defaultTheme = { fontSizeScale: 100, headingColor: '', bodyColor: '', linkAccentColor: '#F47320', fontFamily: 'default' };
  const [t, setT] = useState({ ...defaultTheme, ...(data.globalTheme || {}) });
  ...
  return (
    <div>
      ...
      <div className="adm-card adm-style-card" style={{marginTop:'2rem', borderColor: USE_CLOUD ? '#1FAE5C' : '#F47320'}}>
        <h4 className="adm-style-card__title" style={{color: USE_CLOUD ? '#1FAE5C' : '#F47320'}}>
          {USE_CLOUD ? '✅ Cloud Storage Active' : '⚠️ Cloud Storage Setup (REQUIRED for permanent save)'}
        </h4>
        {!USE_CLOUD ? (
          <div>
            <p className="adm-hint" style={{background:'rgba(244,115,32,0.08)', borderColor:'var(--adm-orange)'}}>
              <strong>Abhi data sirf tumhare browser mein save ho raha hai</strong> — refresh karo ya doosra device use karo, data wapas default ho jayega. Permanent save ke liye JSONBin.io setup karo (FREE, 2 min kaam):
            </p>
            <div className="adm-steps">
              <div className="adm-step"><span>1</span><div><strong>jsonbin.io</strong> pe jaao → Free account banao</div></div>
              <div className="adm-step"><span>2</span><div><strong>"+ CREATE BIN"</strong> click karo → Content mein <code>{'{}'}</code> likhke Save karo</div></div>
              <div className="adm-step"><span>3</span><div>Bin ka <strong>BIN ID</strong> copy karo (URL mein dikhega, jaise: <code>64abc123...</code>)</div></div>
              <div className="adm-step"><span>4</span><div>Left sidebar mein <strong>"API Keys"</strong> → Master Key copy karo</div></div>
              <div className="adm-step"><span>5</span><div><code>src/context/SiteDataContext.jsx</code> file kholo aur yeh 2 lines update karo:</div></div>
            </div>
            <pre className="adm-code">{`const JSONBIN_BIN_ID  = 'YAHAN_APNA_BIN_ID_DAALO';
const JSONBIN_API_KEY = 'YAHAN_APNI_API_KEY_DAALO';`}</pre>
            <p style={{color:'#5A6478', fontSize:'0.85rem', marginTop:'0.75rem'}}>Iske baad GitHub pe push karo → Vercel redeploy → ✅ Permanent cloud save active!</p>
          </div>
        ) : (
          <p className="adm-hint" style={{background:'rgba(31,174,92,0.08)', borderColor:'#1FAE5C', color:'#1FAE5C'}}>
            ✅ JSONBin cloud connected hai. Har save ke baad data cloud mein permanently store hoga — kisi bhi device se admin panel kholo, same data milega.
          </p>
        )}
      </div>
    </div>
      ...
      <main className="adm-main">
        ...
        {!USE_CLOUD && (
          <div className="adm-cloud-notice">
            <strong>⚠️ Cloud Storage Setup Required</strong> — Abhi data sirf is browser mein save ho raha hai. Doosre device ya refresh ke baad data ja sakta hai.
            <br/>Setup ke liye <strong>"🌐 Global Theme"</strong> tab → "Cloud Storage Setup" section dekho.
          </div>
        )}
